...
class _ServiceDetailScreenState extends State<ServiceDetailScreen> {
  ...
  Service get _current {
    ...
    return services.where((item) => item.url == widget.service.url).firstOrNull ?? widget.service;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        Card(color: color.withOpacity(.12), child: Padding(padding: const EdgeInsets.all(16), child: Row(children: [
          Icon(Icons.circle, color: color, size: 28), const SizedBox(width: 12),
          Expanded(child: Text(label, style: TextStyle(color: color, fontSize: 20, fontWeight: FontWeight.bold))),
          Chip(label: Text(label), backgroundColor: color.withOpacity(.2)),
        ]))),
        _section('Endpoint surveillé', SelectableText(service.url)),
        _section('Performance', Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('${service.responseTime} ms', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8), LinearProgressIndicator(value: gauge, minHeight: 10, color: color, backgroundColor: Colors.grey.shade300),
          const SizedBox(height: 6), Text(service.responseTime < 800 ? 'Latence normale' : 'Latence élevée'),
        ])),
        _section('Informations techniques', Column(children: [
          _infoRow('Réponse', httpCode),
          _infoRow('Disponibilité estimée', service.status == ServiceStatus.down ? 'Indisponible' : '99,9%'),
          _infoRow('Dernière vérification', _formatDate(service.lastChecked)),
        ])),
        _section('Vérifications récentes', _timeline(service, color)),
  ...
  Widget _timeline(Service service, Color color) => Column(children: List.generate(5, (index) => ListTile(contentPadding: EdgeInsets.zero, leading: Icon(Icons.check_circle, color: color, size: 20), title: Text('${service.responseTime + index * 7} ms'), subtitle: Text(index == 0 ? 'Dernier contrôle' : '${index * 2} min auparavant'))));
