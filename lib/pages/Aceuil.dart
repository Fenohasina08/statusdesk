...
class _AceuilState extends State<Aceuil> {
  ...
  Widget build(BuildContext context) {
    ...
    final overall = down > 0 ? 'Certains services sont indisponibles' : degraded > 0 ? 'Performances dégradées' : services.isEmpty ? 'Aucun service surveillé' : 'Tous les systèmes sont opérationnels';
    ...
    return Scaffold(
      body: RefreshIndicator(
        ...
        child: ListView(padding: const EdgeInsets.fromLTRB(10, 50, 10, 20), children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Status Desk', style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold)), IconButton(onPressed: () => context.read<ServiceProvider>().fetchServices(), icon: const Icon(Icons.refresh))]),
          ...
          if (provider.isLoading && services.isEmpty) const Center(child: Padding(padding: EdgeInsets.all(32), child: CircularProgressIndicator()))
          else if (provider.error != null && services.isEmpty) _ErrorState(message: provider.error!, onRetry: () => context.read<ServiceProvider>().fetchServices())
          else if (services.isEmpty) const Center(child: Padding(padding: EdgeInsets.all(24), child: Text('Aucun service disponible.')))
          else ...services.map((service) => _ServiceTile(service: service)),
        ]),
      ),
    );
  }
...
class _CounterCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(width: 160, height: 105, padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(10)), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Text(label, style: const TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.bold)), const SizedBox(height: 8), Text('$value', style: const TextStyle(color: Colors.white, fontSize: 30, fontWeight: FontWeight.bold))]));
}
...
class _ServiceTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(child: ListTile(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ServiceDetailScreen(service: service))), leading: Icon(Icons.circle, color: color), title: Text(service.name, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('$label • ${service.responseTime} ms'), trailing: const Icon(Icons.arrow_forward_ios, size: 16)));
  }
}
